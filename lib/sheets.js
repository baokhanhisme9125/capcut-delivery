...
/* ─────────────────────────────────────────────────────────────
   ...
   D: SoldAt | E: OrderID | F: ProductType | G: ProductName | H: Platform
───────────────────────────────────────────────────────────── */

async function saveOrder({ uniqueCode, buyerEmail, accountEmail, accountPassword, orderId, productType, productName, platform }) {
  const sheets = await getSheetsClient();
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        uniqueCode,
        buyerEmail,
        `${accountEmail}:${accountPassword}`,   // merged into 1 cell
