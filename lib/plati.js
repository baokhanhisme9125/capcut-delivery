 ...
 * This API is NOT behind DDoS-Guard unlike plati.market/api/api.ashx
 *
 * Flow:
...
async function getToken() {
  ...
  const res  = await fetch(`${DIGI_API}/apilogin`, {
    method:  'POST',
...
async function verifyUniqueCode(uniqueCode) {
  ...
  const ucRes  = await fetch(`${DIGI_API}/purchases/unique-code/${uniqueCode}?token=${token}`, {
  ...
  try {
    const piRes  = await fetch(`${DIGI_API}/purchase/info/${ucData.inv}?token=${token}`, {
      headers: { 'Accept': 'application/json' },
      timeout: 12000,
    });
