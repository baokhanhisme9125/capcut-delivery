...
async function verifyUniqueCode(uniqueCode) {
  ...
  // Build form body (Plati API accepts POST with x-www-form-urlencoded)
  const body = new URLSearchParams({
    action:      'getInfoUniqueCode',
    id_goods:    idGoods,
  ...
  let rawText = '';
  try {
    const res  = await fetch(PLATI_API, {
      method:  'POST',
    ...
    // Include first 200 chars of raw response to help debug
    const preview = rawText ? ` | Response: ${rawText.slice(0, 200)}` : '';
    throw new Error(`Plati API error: ${err.message}${preview}`);
  }
